...
function autocomplete(input, latInput, lngInput) {
  ...
  // if someone hits enter in the address field don't submit form
...
function loadPlaces(map) {
  ...
  var lat = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 43.2;

  _axios2.default.get('/api/stores/near?lng=' + lng + '&lat=' + lat).then(function (res) {
    var places = res.data;
...
var dompurify = __webpack_require__(31);
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        var html = dompurify.sanitize(searchResultsHTML(res.data));
